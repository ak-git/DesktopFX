...
public class SecondDerivativeApparentByPhiPhi2RhoTest {
  ...
  public void testValueSL(@Nonnull double[] rho, @Nonnegative double hmm, @Nonnegative double smm, @Nonnegative double lmm, @Nonnegative double rOhm) {
    ...
    actual /= dPhi;
    Assert.assertEquals(actual, Apparent2Rho.newSecondDerivativeApparentByPhiPhi2Rho(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi)), 0.6);
  }
  ...
  public void testValueLS(@Nonnull double[] rho, @Nonnegative double hmm, @Nonnegative double smm, @Nonnegative double lmm, @Nonnegative double rOhm) {
    ...
    double actual = Apparent2Rho.newDerivativeApparentByPhi2Rho(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi + dPhi)) -
        Apparent2Rho.newDerivativeApparentByPhi2Rho(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi));
    actual /= dPhi;
    Assert.assertEquals(actual, Apparent2Rho.newSecondDerivativeApparentByPhiPhi2Rho(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi)), Math.abs(actual / 10.0));
  }
}