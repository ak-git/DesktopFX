...
class SecondDerivativeApparentByPhiPhi2RhoTest {
  ...
  void test() {
    ToDoubleFunction<RelativeMediumLayers> operator =
        Apparent2Rho.newSecondDerivativeApparentByPhiPhiDivRho1(new RelativeTetrapolarSystem(10.0 / 30.0));
  ...
  void testValueSL(@Nonnull double[] rho, @Nonnegative double hmm, @Nonnegative double smm, @Nonnegative double lmm) {
    ...
    assertThat(actual).isCloseTo(
        Apparent2Rho.newSecondDerivativeApparentByPhiPhiDivRho1(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi)),
        byLessThan(0.6));
  }
  ...
  void testValueLS(@Nonnull double[] rho, @Nonnegative double hmm, @Nonnegative double smm, @Nonnegative double lmm) {
    ...
    double dPhi = 1.0e-6 * phi;
    double k12 = Layers.getK12(rho[0], rho[1]);
    double actual = Apparent2Rho.newDerivativeApparentByPhiDivRho1(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi + dPhi)) -
        Apparent2Rho.newDerivativeApparentByPhiDivRho1(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi));
    actual /= dPhi;
    assertThat(actual).isCloseTo(
        Apparent2Rho.newSecondDerivativeApparentByPhiPhiDivRho1(system.relativeSystem()).applyAsDouble(new Layer2RelativeMedium(k12, phi)),
