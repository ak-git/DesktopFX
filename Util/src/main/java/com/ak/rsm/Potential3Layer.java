...
final class Potential3Layer extends AbstractPotentialLayer {
  ...
  public double value(@Nonnegative double rho1, @Nonnegative double rho2, @Nonnegative double rho3, @Nonnegative int p1, @Nonnegative int p2mp1) {
    ...
    double k12 = Potential2Layer.getK12(rho1, rho2);
    double k23 = Potential2Layer.getK12(rho2, rho3);
    ...
    double[] q = CoefficientsUtils.serialize(bNum, aDen, Potential2Layer.SUM_LIMIT + 1);
    return value(rho1, r -> (1.0 / r + 2.0 * Potential2Layer.sum(n -> q[n], Potential2Layer.denominator(r, hStep))));
  }
}
