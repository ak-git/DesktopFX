...
class AperStage6Current1Variable7mmTest {
  static Stream<Arguments> variables() {
    return Stream.of(
        arguments(new byte[] {1,
            ...
            new int[] {55318, 5982, 330990, 5982, 2433, 9705, 1302}
        )
    );
  }
  ...
  void testApply(byte[] inputBytes, int[] outputInts) {
    Converter<BufferFrame, AperStage6Current1Variable7mm> converter = LinkedConverter
        ...
        .chainInstance(AperStage6Current1Variable7mm.class);
  ...
  @ParameterizedTest
  @EnumSource(value = AperStage6Current1Variable7mm.class)
  void testGetInputVariables(@Nonnull DependentVariable<AperStage5Current1Variable, AperStage6Current1Variable7mm> variable) {
    assertThat(variable.getInputVariables()).hasSize(1);
  }

  @Test
  void testGetUnit() {
    assertThat(EnumSet.allOf(AperStage6Current1Variable7mm.class).stream().map(DependentVariable::getUnit))
        .isEqualTo(
            List.of(
  ...
  @Test
  void testOptions() {
    assertThat(EnumSet.allOf(AperStage6Current1Variable7mm.class).stream().flatMap(v -> v.options().stream()))
        .isEqualTo(
            List.of(
  ...
  @ParameterizedTest
  @EnumSource(value = AperStage6Current1Variable7mm.class)
  void testFilterDelay(@Nonnull Variable<AperStage6Current1Variable7mm> variable) {
    assertThat(variable.filter().getDelay()).isZero();
  }

  @ParameterizedTest
  @EnumSource(value = AperStage6Current1Variable7mm.class)
  void testInputVariablesClass(@Nonnull DependentVariable<AperStage5Current1Variable, AperStage6Current1Variable7mm> variable) {
