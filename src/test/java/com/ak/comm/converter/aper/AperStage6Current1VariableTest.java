...
public class AperStage6Current1VariableTest {
  ...
  public void testGetUnit() {
    List<? extends Unit<?>> actual = EnumSet.allOf(AperStage6Current1Variable.class).stream()
        .map(DependentVariable::getUnit).collect(Collectors.toList());
    Assert.assertEquals(actual,
        Arrays.asList(
            OHM.multiply(METRE), OHM.multiply(METRE),
            OHM
        ),
        actual.toString()
    );
  ...
  public void testOptions() {
    List<Variable.Option> actual = EnumSet.allOf(AperStage6Current1Variable.class).stream()
