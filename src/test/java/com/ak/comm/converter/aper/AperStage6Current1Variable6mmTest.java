...
class AperStage6Current1Variable6mmTest {
  static Stream<Arguments> variables() {
    return Stream.of(
        arguments(new byte[] {1,
            ...
            new int[] {55318, 5982, 330990, 5982, 2085, 8319, 1302}
        )
    );
  }
  ...
  void testApply(byte[] inputBytes, int[] outputInts) {
    Converter<BufferFrame, AperStage6Current1Variable6mm> converter = LinkedConverter
        ...
        .chainInstance(AperStage6Current1Variable6mm.class);
  ...
  @ParameterizedTest
  @EnumSource(value = AperStage6Current1Variable6mm.class)
  void testGetInputVariables(@Nonnull DependentVariable<AperStage5Current1Variable, AperStage6Current1Variable6mm> variable) {
    assertThat(variable.getInputVariables()).hasSize(1);
  }

  @Test
  void testGetUnit() {
    assertThat(EnumSet.allOf(AperStage6Current1Variable6mm.class).stream().map(DependentVariable::getUnit))
        .isEqualTo(
            List.of(
  ...
  @Test
  void testOptions() {
    assertThat(EnumSet.allOf(AperStage6Current1Variable6mm.class).stream().flatMap(v -> v.options().stream()))
        .isEqualTo(
            List.of(
  ...
  @ParameterizedTest
  @EnumSource(value = AperStage6Current1Variable6mm.class)
  void testFilterDelay(@Nonnull Variable<AperStage6Current1Variable6mm> variable) {
    assertThat(variable.filter().getDelay()).isZero();
  }

  @ParameterizedTest
  @EnumSource(value = AperStage6Current1Variable6mm.class)
  void testInputVariablesClass(@Nonnull DependentVariable<AperStage5Current1Variable, AperStage6Current1Variable6mm> variable) {
