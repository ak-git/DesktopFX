...
class BrikoStage3VariableTest {
  ...
  void testApply(byte[] inputBytes, int[] outputInts) {
    Converter<BufferFrame, BrikoStage3Variable> converter = LinkedConverter.of(new BrikoConverter(), BrikoStage2Variable.class)
        .chainInstance(BrikoStage3Variable.class);
  ...
  @Test
  void testOptions() {
    assertThat(EnumSet.allOf(BrikoStage3Variable.class).stream().flatMap(v -> v.options().stream()).collect(Collectors.toSet()))
        .containsExactly(VISIBLE);
  }

  @ParameterizedTest
  @EnumSource(value = BrikoStage3Variable.class)
  void testFilterDelay(@Nonnull Variable<BrikoStage3Variable> variable) {
  ...
  @Test
  void testGetUnit() {
    assertThat(EnumSet.allOf(BrikoStage3Variable.class).stream().map(Variable::getUnit))
        .isEqualTo(List.of(Units.GRAM, Units.GRAM, MILLI(METRE)));
  }

  @ParameterizedTest
  @EnumSource(value = BrikoStage3Variable.class)
  void testInputVariablesClass(@Nonnull DependentVariable<BrikoStage2Variable, BrikoStage3Variable> variable) {
    assertThat(variable.getInputVariablesClass()).isEqualTo(BrikoStage2Variable.class);
  }
}