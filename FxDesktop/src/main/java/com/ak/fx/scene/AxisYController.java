...
final class AxisYController<EV extends Enum<EV> & Variable<EV>> {
  ...
  ScaleYInfo<EV> scale(@Nonnull EV variable, @Nonnull int[] values) {
    IntSummaryStatistics statistics = IntStream.of(values).parallel().summaryStatistics();
    if (statistics.getMax() == statistics.getMin()) {
      statistics = IntStream.of(statistics.getMax(), 0).summaryStatistics();
    }

    int mean = 0;
    if (!variable.options().contains(Variable.Option.FORCE_ZERO_IN_RANGE)) {
      int meanScaleFactor10 = scaleFactor10(mmHeight, statistics.getMax() - statistics.getMin()) * 10;
      mean = (int) Math.rint((statistics.getMax() + statistics.getMin()) / 2.0 / meanScaleFactor10) * meanScaleFactor10;
    }
    int signalRange = Math.max(Math.abs(statistics.getMax() - mean), Math.abs(statistics.getMin() - mean)) * 2;
